    ...
    methods: {
        ...
        queryStatData: function () {
            ajaxGet("article", "/strategies/stat/data", {id: this.strategy.id}, function (data) {
                ...
                vue.strategy.likesnum = map.likesnum;
            })
        },
        //攻略点赞
        strategyLikes: function () {
            ajaxPost("u", "/users/likes", {articleId: vue.strategy.id, type : 1}, function (data) {
                if (data.data) {
                    popup("顶成功啦");
                    // 刷新统计数据
                    vue.queryStatData();
        ...
        //攻略收藏
        favor: function () {
            ajaxPost("u", "/users/favor", {articleId: vue.strategy.id, type: 1}, function (data) {
                if (data.data) {
                    popup("收藏成功");
        ...
        //评论点赞
        commentLikes: function (commentId) {
        ...
        //点赞小手
        thumbEcho: function (likesList) {
            if (user) {
                var id = parseInt(user.id)
                return $.inArray(id, likesList) !== -1;
            }
        }
    },
    filters: {
