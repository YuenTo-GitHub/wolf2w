var vue = new Vue({
    ...
    methods:{
        ...
        phoneCheck:function (){
            ...
            if (/^1(3\d|4[5-9]|5[0-35-9]|6[567]|7[0-8]|8\d|9[0-35-9])\d{8}$/g.test(val)) {
                $.get(getServiceUrl("us") + "/users/checkPhone", {phone: val}, function (data) {
                    if (data.code === 200) {
                        if (!data.data) {
        ...
        sendCode: function (event) {
            ...
            if (_this.hasClass('disabled')) {
                ...
                $.get(getServiceUrl("u") + "/users/sendVerifyCode", {phone: phone, type: 1}, function (data) {
                    if (data.code === 200) {
                        popup("发送成功")
                    }else{
                        popup(data.msg);
